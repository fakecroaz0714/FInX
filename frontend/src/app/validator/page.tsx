...
export default function ValidatorDashboard() {
    ...
    const [activeTab, setActiveTab] = useState<'Overview' | 'Validation Queue' | 'Risk Assessment' | 'Project Proposals' | 'Audit Logs'>('Overview');
            ...
            <Card className="border border-slate-200 shadow-sm overflow-hidden">
                <CardHeader className="bg-slate-50 border-b border-slate-100 p-0 px-6 pt-3 pb-0">
                    <div className="flex items-center gap-6 overflow-x-auto hide-scrollbar">
                        {(['Overview', 'Validation Queue', 'Risk Assessment', 'Project Proposals', 'Audit Logs'] as const).map(tab => (
                            <button
                                key={tab}
                                onClick={() => setActiveTab(tab)}
                                className={`text-sm font-semibold pb-3.5 pt-2 border-b-2 transition-colors whitespace-nowrap cursor-pointer ${
                                    activeTab === tab
